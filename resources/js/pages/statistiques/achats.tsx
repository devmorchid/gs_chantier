...
export default function StatistiquesAchats({ stats, filters = {} }: { stats: any, filters?: any }) {
  ...
  const handleFilter = (e: React.FormEvent) => {
    e.preventDefault();
    router.get('/statistiques/achats', { mode, year, month, date_debut: dateDebut, date_fin: dateFin }, { preserveState: true });
  };

  return (
    <AppLayout breadcrumbs={[{ title: 'Dashboard', href: '/dashboard' }, { title: 'Statistiques', href: '/statistiques' }, { title: 'Achats', href: '/statistiques/achats' }]}> 
      <Head title="Statistiques Achats" />
      <div className="flex flex-col gap-6 p-6">
        ...
        <h1 className="text-2xl font-bold mb-4">Statistiques Achats</h1>
        <form onSubmit={handleFilter} className="flex flex-wrap gap-4 items-end mb-4">
          ...
          <button type="submit" className="px-4 py-2 rounded bg-green-700 text-white hover:bg-green-600 transition">Filtrer</button>
        </form>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          <div className="bg-zinc-900 rounded-lg shadow p-6 flex flex-col items-center border border-green-700">
            <div className="text-4xl font-bold text-green-400">{stats.total_montant?.toLocaleString()} DH</div>
            <div className="mt-2 text-sm text-green-200">Total achats</div>
          </div>
          <div className="bg-zinc-900 rounded-lg shadow p-6 flex flex-col items-center border border-blue-600">
            ...
            <div className="mt-2 text-sm text-blue-200">Nombre d'achats</div>
          </div>
        </div>
        <div className="bg-zinc-900 rounded-lg shadow p-6 mt-8 w-full">
          <h2 className="text-lg font-semibold mb-2 text-center text-white">Évolution des achats</h2>
          <Bar
            data={{
              labels: stats.labels || [],
              datasets: [
