...
export default function ProduitsEdit({ produit, categories, fournisseurs }: Props) {
  ...
  const initialProfitPct = computeProfitPercent(
    parseNumber(produit.prix_achat),
    parseNumber(produit.prix_vente),
  );

  const [form, setForm] = useState({
    code_barre: produit.code_barre || '',
    name: produit.name || '',
    category_name: produit.category?.name || '',
    category_id: produit.category_id ? String(produit.category_id) : '',
    prix_achat: produit.prix_achat || '',
    prix_vente: produit.prix_vente || '',
    profit_pct: initialProfitPct,
    fournisseur_name: produit.fournisseur?.name || '',
    fournisseur_id: produit.fournisseur_id ? String(produit.fournisseur_id) : '',
    quantite: produit.quantite || '',
    image: produit.image || '', // string for display, File for upload
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(`/produits/${produit.id}?_method=PUT`, data, {
      forceFormData: true,
                      ...
                      <img
                        src={
                          form.image && typeof form.image === 'string'
                            ? (form.image.startsWith('http') ? form.image : `/storage/${form.image}`)
                            : defaultImage
                        }
                        alt="Produit"
                        className="w-32 h-32 object-cover rounded border"
