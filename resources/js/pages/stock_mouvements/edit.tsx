...
export default function StockMouvementsEdit({ produits, chantiers, destinationChantiers, isChefChantier, produitsByChantier, mouvement }: Props) {
  const { data, setData, patch, processing, errors } = useForm<StockMouvementForm>({
    type: mouvement.type,
    origine: mouvement.origine ?? '',
    destination: mouvement.destination,
    date: mouvement.date,
    items: mouvement.items?.length
      ? mouvement.items.map((item) => ({ produit: item.produit?.name ?? '', quantite: item.quantite }))
      : [{ produit: mouvement.produit?.name ?? '', quantite: mouvement.quantite }],
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    patch(`/stock-mouvements/${mouvement.id}`, data as unknown as Record<string, any>);
  }
    ...
    <AppLayout>
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-tight">Modifier un mouvement de stock</h1>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="w-full mx-auto">
            <CardHeader>
              ...
              <CardDescription>
                Mettez à jour les informations principales du mouvement de stock
              </CardDescription>
            </CardHeader>
              ...
              <div className="rounded-lg border border-border/60 bg-muted/20 p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Optionnel, utile pour tracer le flux.</p>
