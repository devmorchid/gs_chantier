...
interface Props {
  fournisseur: FournisseurForm & { id: number };
}

export default function FournisseursEdit({ fournisseur }: Props) {
  const { data, setData, put, processing, errors } = useForm<FournisseurForm>({
    type: fournisseur.type,
    name: fournisseur.name || '',
    rc: fournisseur.rc || '',
    ice: fournisseur.ice || '',
    if_fiscal: fournisseur.if_fiscal || '',
    tp: fournisseur.tp || '',
  ...
  return (
    <AppLayout>
      <Head title={`Modifier ${fournisseur.name}`} />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="w-full mx-auto">
            <CardHeader>
              ...
              <CardDescription>Mettez à jour les informations du fournisseur</CardDescription>
            </CardHeader>
