...
<body>
        ...
        <table class="header-table">
            <tr>
                ...
                <td width="30%" class="text-center">
                    ...
                    <div style="font-size: 20px; margin-top: 15px; font-weight: bold; text-decoration: underline;">BON D'ACHAT</div>
                </td>
    ...
    <div class="container">
        <table class="info-section">
            <tr>
                <td class="info-box">
                    <div class="box-header">FOURNISSEUR</div>
                    <div class="box-content">
                        <strong>Nom :</strong> {{ $achat->fournisseur?->name ?? '-' }}<br>
                        <strong>Tél :</strong> {{ $achat->fournisseur?->phone ?? '-' }}<br>
                        <strong>Ville :</strong> {{ $achat->fournisseur?->ville ?? '-' }}
                    </div>
                </td>
                <td width="4%"></td>
                <td class="info-box">
                    ...
                    <div class="box-content">
                        <strong>Achat N° :</strong> {{ $achat->reference ?? '#' . $achat->id }}<br>
                        <strong>Date :</strong> {{ $achat->date->format('d/m/Y') }}<br>
                        <strong>Statut :</strong> {{ $achat->statut_label }}<br>
                        @if($achat->notes)<strong>Notes :</strong> {{ $achat->notes }}@endif
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="items-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">{{ number_format($item->prix_achat * $item->quantite, 2, ',', ' ') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div style="margin-top: 20px;">
            <table align="right" style="width: 38%; border-collapse: collapse;">
                <tr>
                    ...
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($achat->total_ht, 2, ',', ' ') }}</td>
                </tr>
                <tr>
                    <td style="border: 1.5px solid #000; padding: 7px;"><strong>TVA {{ (int) $achat->tva_rate }} %</strong></td>
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($achat->total_tva, 2, ',', ' ') }}</td>
                </tr>
                <tr style="background: #f2f2f2;">
                    <td style="border: 1.5px solid #000; padding: 7px;"><strong>TOTAL TTC</strong></td>
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($achat->total_ttc, 2, ',', ' ') }}</td>
                </tr>
            </table>
        </div>
    </div>
