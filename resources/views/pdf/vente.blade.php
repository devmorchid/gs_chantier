        ...
        <table class="header-table">
            <tr>
                ...
                <td width="30%" class="text-center">
                    ...
                    <div style="font-size: 20px; margin-top: 15px; font-weight: bold; text-decoration: underline;">BON DE VENTE</div>
                </td>
            ...
            <tr>
                <td class="info-box">
                    ...
                    <div class="box-content">
                        <strong>Nom :</strong> {{ $vente->client?->name ?? '-' }}<br>
                        <strong>Tél :</strong> {{ $vente->client?->phone ?? '-' }}<br>
                        <strong>Ville :</strong> {{ $vente->client?->ville ?? '-' }}
                    </div>
                </td>
                <td width="4%"></td>
                <td class="info-box">
                    <div class="box-header">INFOS VENTE</div>
                    <div class="box-content">
                        <strong>Vente N° :</strong> {{ $vente->reference ?? '#' . $vente->id }}<br>
                        <strong>Date :</strong> {{ $vente->date->format('d/m/Y') }}<br>
                        <strong>Statut :</strong> {{ $vente->statut_label }}<br>
                        @if($vente->notes)<strong>Notes :</strong> {{ $vente->notes }}@endif
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="items-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right">{{ number_format($item->prix_vente * $item->quantite, 2, ',', ' ') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div style="margin-top: 20px;">
            <table align="right" style="width: 38%; border-collapse: collapse;">
                <tr>
                    ...
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($vente->total_ht, 2, ',', ' ') }}</td>
                </tr>
                <tr>
                    <td style="border: 1.5px solid #000; padding: 7px;"><strong>TVA {{ (int) $vente->tva_rate }} %</strong></td>
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($vente->total_tva, 2, ',', ' ') }}</td>
                </tr>
                <tr style="background: #f2f2f2;">
                    <td style="border: 1.5px solid #000; padding: 7px;"><strong>TOTAL TTC</strong></td>
                    <td class="text-right" style="border: 1.5px solid #000; padding: 7px;">{{ number_format($vente->total_ttc, 2, ',', ' ') }}</td>
                </tr>
            </table>
        </div>
    </div>
